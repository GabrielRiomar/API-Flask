from flask import Blueprint, request, jsonify
from models.task import Task
import storage

# blueprint for task routes
task_bp = Blueprint('task', __name__)

# Create a new task
@task_bp.route('/tasks', methods=['POST'])
def create_task():
  data = request.get_json()
  new_task = Task(id=storage.task_id_counter, title=data['title'], description=data.get('description', ''), completed=False)
  storage.task_id_counter += 1
  storage.tasks.append(new_task)
  return {'id': new_task.id, 'title': new_task.title, 'description': new_task.description, 'completed': new_task.completed}, 201

# Retrieve all tasks
@task_bp.route('/tasks', methods=['GET'])
def get_tasks():
  task_list = [task.to_dict() for task in storage.tasks]
  output = {'tasks': task_list,
            'total': len(task_list)}
  return jsonify(output), 200

# Retrieve a specific task by ID
@task_bp.route('/tasks/<int:id>', methods=['GET'])
def get_task(id):
  for task in storage.tasks:
    if task.id == id:
      return jsonify(task.to_dict()), 200
  return {'message': 'Task not found'}, 404

# Update a task by ID
@task_bp.route('/tasks/<int:id>', methods=['PUT'])
def update_task(id):
  data = request.get_json()
  for task in storage.tasks:
    if task.id == id:
      task.title = data.get('title', task.title)
      task.description = data.get('description', task.description)
      task.completed = data.get('completed', task.completed)
      return jsonify(task.to_dict()), 200
  return {'message': 'Task not found'}, 404

# Delete a task by ID
@task_bp.route('/tasks/<int:id>', methods=['DELETE'])
def delete_task(id):
    for t, task in enumerate(storage.tasks):
        if task.id == id:
            storage.tasks.pop(t)
            return {'message': 'Task deleted successfully'}, 200
    return {'message': 'Task not found'}, 404